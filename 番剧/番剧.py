...
def getData():
    ...
    for j in range(1, 20):
        ...
        url = 'https://api.bilibili.com/pgc/season/index/result?season_version=-1&spoken_language_type=-1&area=-1' \
              '&is_finish=-1&copyright=-1&season_status=-1&season_month=-1&year=-1&style_id=-1&order=2&st=1&sort=0' \
              '&page='+str(j)+'&season_type=1&pagesize=20&type=1 '
        html = askURL(url)
        ...
        for i in range(len(json_list)):
            ...
            # 具体视频页链接
            link = json_list[i]['link']
            specific_id = str(json_list[i]['media_id'])
            url = 'https://api.bilibili.com/pgc/review/user?media_id=' + specific_id
            # 访问具体动态页面
...
def saveData(data_list, save_path):
    ...
    # 创建工作表
    sheet = book.add_sheet('番剧', cell_overwrite_ok=True)
    col = ("番剧名称", "番剧链接", "资格", "播放数", "国家", "简介", "演员", "类型", "评分", "能否下载")
    for i in range(0, 10):
        # 列名
