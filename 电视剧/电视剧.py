...
def getData():
    ...
    for j in range(1, 20):
        ...
        url = 'https://api.bilibili.com/pgc/season/index/result?area=-1&style_id=-1&release_date=-1&season_status=-1' \
              '&order=2&st=5&sort=0&page=' + str(j) + '&season_type=5&pagesize=20&type=1 '
        html = askURL(url)
...
def saveData(data_list, save_path):
    ...
    # 创建工作表
    sheet = book.add_sheet('电视剧', cell_overwrite_ok=True)
    col = ("电视剧名称", "电视剧链接", "资格", "播放数", "国家", "简介", "演员", "类型", "评分", "能否下载")
    for i in range(0, 10):
        # 列名
